...
public class PageDto {
	...
	public PageDto(int total, Criteria criteria) {
		...
		//위 식으로 페이지 값이 1 ~ 10 일때 endPate 값이 10으로 셋팅
		//위 식으로 페이지 값이 11 ~ 20 일때 endPate 값이 20으로 셋팅		
		//위 식으로 페이지 값이 31 ~ 30 일때 endPate 값이 30으로 셋팅
		...
		if(realEndPage < this.endPage) { // 실제 페이지의 개수가 계산한 endPage 수 보다 작을 때만 교체
			this.endPage = realEndPage;
		}
		
		this.prev = this.startPage > 1; // startPage 값이 1만 ㅏ니면 prev 가 True 로 저장되어야 함
		this.next = realEndPage > this.endPage;
		// 마지막 페이지 번호가 실제 페이지 번호보다 작은 경우에만 next 가 true 로 저장되어야 함
